...
#
# Creates a prefix env at /project/ikoutis/conda_env/dml-torch (override by
# exporting DML_CONDA_ENV first) with python 3.11 and installs
# requirements.txt. The torch/torchvision wheels on PyPI bundle the CUDA 12
# runtime, so no cluster CUDA module is needed — the driver on the GPU
# nodes is enough. Project storage keeps the env off the $HOME quota and
# shareable within the group.
#
...
#          bash -lc 'module load Miniforge3 && source "$(conda info --base)/etc/profile.d/conda.sh" && conda activate "${DML_CONDA_ENV:-/project/ikoutis/conda_env/dml-torch}" && python -c "import torch; print(torch.__version__, torch.cuda.is_available(), torch.cuda.get_device_name(0))"'
# =============================================================================
set -euo pipefail

ENV_PREFIX="${DML_CONDA_ENV:-/project/ikoutis/conda_env/dml-torch}"
...
if [ "$ENV_PREFIX" != "/project/ikoutis/conda_env/dml-torch" ]; then
